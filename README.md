...
Cette section décrit l’architecture en couches telle que demandée, avec une séparation claire Frontend / Backend et une interface matérielle.

- HUAHINE.py
  - Rôle: point d’entrée et fenêtre principale (PyQt5). Initialise les services, la carte (Quart), et orchestre les modules.
  - Expose les actions UI (ouvrir/fermer CAN, lire/stop, status, export/import, carte, aide, etc.).

Backend (métier et I/O):
- Package/backend/can_backend.py
  - Classe CanBackend: surcouche légère autour de Package/CAN_dll.CANDll
  - Responsabilités:
    - Ouverture/fermeture de la liaison
    - Accès au status
    - Lecture asynchrone une fois (read_once) ou en boucle (read_forever) avec callback sur chaque trame
    - Écriture (send) proxy vers la DLL
  - Objectif: fournir une API propre et testable, et décorréler l’UI de la DLL.

- Package/CANApplication.py
  - Composant « application CAN » côté UI, utilise désormais CanBackend (tout en restant compatible avec CANDll).
  - Actions:
    - Ouverture / fermeture
    - Lancement/arrêt de la lecture (asyncio)
    - Consultation du Status (FenetreStatus)
    - Déclenche l’envoi (via backend/dll)

- Package/CAN_dll.py
  - Accès bas niveau à la DLL `canusbdrv64.dll` (port USB FTDI / adaptateur CANUSB).
  - Fournit les appels C (open/close/read/status/flush/write) via ctypes et la structure CanMsg.

- Package/TempsReel.py
  - Réception en temps réel: écrit le fichier texte si coché, met à jour le buffer UI si demandé, et appelle NMEA_2000.octets(...) si activé.

- Package/NMEA_2000.py
  - Conversion NMEA2000: décode ID → PGN, Priorité, Source, Dest; interprète les octets.
  - Expose utilitaires (participants, AIS, coordonnées, instruments, etc.).

- Package/MMSI.py
  - Gestion/lookup des MMSI pour l’AIS.

Frontend (web embarqué):
- static/js/mon.js
  - Affiche la carte dans le webbrowser intégré (via Quart). Fait les requêtes API: tuiles, nav, AIS, historique, routes, etc.
- templates/index.html
  - Page carte (servie par Quart dans HUAHINE.py).

Notes d’intégration:
- L’UI continue de fonctionner comme avant. CanBackend a été ajouté sans casser l’existant; CANApplication instancie CanBackend et expose toujours l’instance CANDll pour compatibilité.
- Les modules pourront progressivement migrer vers CanBackend (API stable open/close/status/read_once/read_forever/send).
- Aucun changement n’est requis pour la DLL ou pour NMEA_2000.
